    ...
    vector<string> braceExpansionII(string expression) {
        ...
        for(auto&x:expression){
            if(x >= 'a' and x <= 'z'){
                ...
                for(auto&i:curr) res.push_back(i); // Add all elements in curr to res
                curr.clear(); // Clear curr before moving on
            }
        }
        for(auto&i:curr) res.push_back(i); // Add any remaining elements in curr to res
